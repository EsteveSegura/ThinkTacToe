...
MODELS_CONFIG = [
    ...
    {
        'name': 'light_grpo_run',
        'path': '/home/ThinkTacToe/light_grpo_run/checkpoint-1000',
        'has_think': True
    },
]
...
def create_prompt_minimax(board: list, player: str = 'X') -> str:
    """
...
def test_model(model_config: dict, num_games: int = 10) -> pd.DataFrame:
    ...
    for game in range(num_games):
        ...
        # Crear prompt en formato minimax
        prompt = create_prompt_minimax(board, 'X')
    ...
    print(f"\nResumen para {model_name} (formato minimax):")
...
if __name__ == "__main__":
    ...
    for model_config in MODELS_CONFIG:
        try:
            ...
            if not df.empty:
                ...
                individual_file = results_dir / f'{model_name_clean}_minimax_evaluation_{timestamp}.csv'
    ...
    if all_results:
        ...
        combined_file = results_dir / f'all_models_minimax_evaluation_{timestamp}.csv'
        combined_df.to_csv(combined_file, index=False)
        
        # Mostrar resumen general
        print(f"\n{'='*80}")
        print("RESUMEN GENERAL DE TODOS LOS MODELOS (FORMATO MINIMAX)")
        print(f"{'='*80}")
