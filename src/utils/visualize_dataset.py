...
def visualize_board(board: List[List[str]], title: str = ""):
    ...
    for i in range(3):
        for j in range(3):
            if board[i][j] == 'X':
                ax.plot([j + 0.2, j + 0.8], [2.8 - i, 2.2 - i], 'r-', linewidth=3)
                ax.plot([j + 0.2, j + 0.8], [2.2 - i, 2.8 - i], 'r-', linewidth=3)
            elif board[i][j] == 'O':
...
def visualize_random_states(dataset_path: str, num_samples: int = 5, num_visualizations: int = 3, save_path: str = "tmp/visualization"):
...
if __name__ == "__main__":
    visualize_random_states("tictac_dataset.json", num_samples=5, num_visualizations=3) 